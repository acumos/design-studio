...
public class GDMServiceImpl implements IGDMService {
	...
	private static String FIELD_MAPPING_FILE_NAME = "classpath:FieldMapping.json";
	...
	@Override
	public OutputStream mapData(InputStream inputStream) {
	...
	private output.Builder transformInputToOutput(input.Builder inputMessage, DataMap dataMap) throws ConversionException {
		...
		/*List<com.google.protobuf.Descriptors.FieldDescriptor> inputMessagefields = inputMessage.getDescriptor().getFields();
		logger.debug("List of input fields : ");
		for(com.google.protobuf.Descriptors.FieldDescriptor d : inputMessagefields ){
			logger.debug("Full Name : " + d.getFullName());
			logger.debug(" Name : " + d.getName());
			logger.debug(" Type : " + d.getType());
			logger.debug(" JavaType : " + d.getJavaType());
			logger.debug(" Repeated : " + d.isRepeated());
		}
		
		List<com.google.protobuf.Descriptors.FieldDescriptor> outputMessagefields = outputMessage.getDescriptor().getFields();
		logger.debug("List of input fields : ");
		for(com.google.protobuf.Descriptors.FieldDescriptor d : outputMessagefields ){
			logger.debug("Full Name : " + d.getFullName());
			logger.debug(" Name : " + d.getName());
			logger.debug(" Type : " + d.getType());
			logger.debug(" JavaType : " + d.getJavaType());
		...
		for(MapInput mapInput : inputMaps) {
			...
			for(InputField inputField : inputMapFields){
				...
				logger.debug("outputField " + inputField.getMapped_to_field() + " value : " + outputMessage.getField(outputfieldDesc));
			}
		}
