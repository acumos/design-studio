...
public class SolutionServiceImpl implements ISolutionService {
				...
				for (Nodes nodesData : cdumpNodeList) {
					if (nodesData.getNodeId().equals(nodeId)) {
						...
						// to update the DataBroker
							...
							updateDataBroker(databrokerMap, nodesData, properties);
						// to update the CollatorMap
						}if (null != collatorMap && collatorMap.toString().length() != 0) {
							updateCollatorMap(collatorMap, nodesData, properties);
						// to update the SplitterMap
						}if (null != splitterMap && splitterMap.toString().length() != 0) {
							updateSplitterMap(splitterMap, nodesData, properties);
						}
						results = String.format(resultTemplate, true, "");
						break;
					} else {
	 ...
	 * @param properties
	 */
	private void updateSplitterMap(SplitterMap splitterMap, Nodes nodesData, Property[] properties) {
		logger.debug(EELFLoggerDelegator.debugLogger, "------- modifyNode updateSplitterMap() ------- : Begin");
		int countVal =0;
		for (int i = 0; i < properties.length; i++) {
			Property newProp = properties[0];
			if(newProp != null){
				countVal++;
			}
		}
		...
		if (null != properties && properties.length != 0) {
			...
			if(propertyList.size() == 0){ //if the properties is empty then add the new property with splitterMap. 
				propertyList.add(newProperty);
			}else {
				for(Property p : propertyList){
					if(p != null){
					if(null != p.getSplitter_map()){ //else if splitterMap exist and update the same.
						...
						//pList.add(newProperty); Need to check this
						newPropertyList.add(newProperty); //else add the new SplitterMap to non empty properties. 
					}
					}
				}
			}
			properties = new Property[countVal];
			int cnt = 0;
			for(Property p : newPropertyList){
				if(p != null){
				properties[0] = p;
				cnt++;
				}
			}
			logger.debug(EELFLoggerDelegator.debugLogger, "---------- modifyNode updateSplitterMap() --------- : End");
	 ...
	 * @param properties
	 */
	private void updateCollatorMap(CollatorMap collatorMap, Nodes nodesData, Property[] properties) {
		logger.debug(EELFLoggerDelegator.debugLogger, "------- modifyNode updateCollatorMap() ------- : Begin");
		Property newProperty = null;
		if (null != properties && properties.length != 0) {
			newProperty = new Property();
			newProperty.setCollator_map(collatorMap);
			ArrayList<Property> propertyList = new ArrayList<Property>(Arrays.asList(properties));
			if(propertyList.size() == 0){ //if the properties is empty then add the new property with collatorMap map. 
				propertyList.add(newProperty);
			}else {
				for(Property p : propertyList){
					if(p.getCollator_map() != null){ //else if collatorMap exist and update the same.
						...
						propertyList.add(newProperty); //else add the new CollatorMap to non empty properties. 
					}
				}
			}
			...
			logger.debug(EELFLoggerDelegator.debugLogger, "---------- modifyNode updateCollatorMap() -----------  : End");
			nodesData.setProperties(properties);
		}
	}
	 ...
	 * @param properties
	 */
	private void updateDataBroker(DataBrokerMap databrokerMap, Nodes nodesData, Property[] properties) {
		logger.debug(EELFLoggerDelegator.debugLogger, "------- modifyNode updateDataBroker() ------- : Begin");
		Property newProperty = null;
		if(null != properties){
			...
			logger.debug(EELFLoggerDelegator.debugLogger, "--------- modifyNode updateDataBroker() -------  : End");
			nodesData.setProperties(properties);
		}
	}
	...
	private void updateDataMapper(FieldMap fieldmap, Property[] properties) {
		logger.debug(EELFLoggerDelegator.debugLogger, "------- modifyNode updateDataMapper() ------- : Begin");
		if (null != properties && properties.length != 0) {
		...
		logger.debug(EELFLoggerDelegator.debugLogger, "--------- modifyNode updateDataMapper() --------  : End");
	}
				...
				if (nodesList != null && !nodesList.isEmpty()) {
					for (Nodes node : nodesList) {
						if (node.getNodeId().equals(nodeToUpdate)) {
							...
							// Set SplitterMapInput under Properties field in SplitterMap
							if(null != property.getSplitter_map().getMap_inputs() || property.getSplitter_map().getMap_inputs().length != 0){
								propertyArr[0].getSplitter_map().setMap_inputs(property.getSplitter_map().getMap_inputs());
								updateLinkdetails(linkName, linkId, sourceNodeName, sourceNodeId, targetNodeName,
										targetNodeId, sourceNodeRequirement, targetNodeCapabilityName, cdump);
								addedLink = true;
							}
							// Set SplitterMapOutput under Properties field in SplitterMap
							if(null != property.getSplitter_map().getMap_outputs() || property.getSplitter_map().getMap_outputs().length !=0){
								propertyArr[0].getSplitter_map().setMap_outputs(property.getSplitter_map().getMap_outputs());
								updateLinkdetails(linkName, linkId, sourceNodeName, sourceNodeId, targetNodeName,
										targetNodeId, sourceNodeRequirement, targetNodeCapabilityName, cdump);
								addedLink = true;
							}
							
							// Set CollatorMapInput under Properties field in CollatorMap
							if(null != property.getCollator_map().getMap_inputs() || property.getCollator_map().getMap_inputs().length != 0){
								propertyArr[0].getCollator_map().setMap_inputs(property.getCollator_map().getMap_inputs());
								updateLinkdetails(linkName, linkId, sourceNodeName, sourceNodeId, targetNodeName,
										targetNodeId, sourceNodeRequirement, targetNodeCapabilityName, cdump);
								addedLink = true;
							}
							// Set CollatorMapOutput under Properties field in CollatorMap
							if(null != property.getCollator_map().getMap_outputs() || property.getCollator_map().getMap_outputs().length != 0){
								propertyArr[0].getCollator_map().setMap_outputs(property.getCollator_map().getMap_outputs());
								updateLinkdetails(linkName, linkId, sourceNodeName, sourceNodeId, targetNodeName,
										targetNodeId, sourceNodeRequirement, targetNodeCapabilityName, cdump);
								addedLink = true;
							}
	...
	public boolean deleteLink(String userId, String solutionId, String version, String cid, String linkId) {
		...
		try {
			...
			logger.error(EELFLoggerDelegator.errorLogger," IOException in deleteLink() in SolutionServiceImpl", e);
		} catch (Exception e) {
