		...
		if ((buildArgs != null) && (!buildArgs.trim().isEmpty())) {
			...
			for (String arg : split) {
				...
				if (pair.length == 2) {
					...
					logger.error(EELFLoggerDelegator.errorLogger, "Invalid format for " + arg + ". Buildargs should be formatted as key=value");
				}
			}
		}
			...
			BuildImageResultCallback callback = new BuildImageResultCallback() {
				...
				public void onNext(BuildResponseItem item) {
					if (item.getStream() != null)
						logger.info(EELFLoggerDelegator.applicationLogger, "\t" + item.getStream());
					else
						logger.info(EELFLoggerDelegator.applicationLogger, "\t" + item);
					super.onNext(item);
				}
