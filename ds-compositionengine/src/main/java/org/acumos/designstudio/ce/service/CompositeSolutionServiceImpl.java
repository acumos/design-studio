...
public class CompositeSolutionServiceImpl implements ICompositeSolutionService {
	...
	public String saveCompositeSolution(DSCompositeSolution dscs) throws AcumosException, URISyntaxException {
		...
		if (null != dscs.getcId() && null == dscs.getSolutionId()) {
			logger.info("Started implementation of Case 1 : cid is present and SolutionId is null");
			...
			logger.info("Ended implementation of Case 1 : cid is present and SolutionId is null");
		} else if (null == dscs.getcId() && null != dscs.getSolutionId()) {
			logger.info("Started implementation of Case 2 : cid is null and SolutionId is present");
			try {
			...
			logger.info("Ended implementation of Case 2 : cid is null and SolutionId is present");
		} else {
	...
	private String insertCompositeSolution(DSCompositeSolution dscs) throws AcumosException, IOException {
		...
		logger.info("insertCompositeSolution() Begin ");
		...
		try {
			...
			logger.info("1.  Successfully Created the Solution {0} & generated Solution ID : {1}", mlpSolution.getName(), mlpSolution.getSolutionId());
		} catch (Exception e) {
		...
		try {
			...
			logger.info("2. Successfully Created the SolutionRevision :  {} ", mlpSolutionRevision.getRevisionId());
		} catch (Exception e) {
		...
		try {
			...
			if (null == cdump) {
				logger.info("Error : Cdump file not found for Solution ID :   {} ", mlpSolution.getSolutionId());
			} else {
				...
				logger.info("3. Successfully read the Cdump file for solution ID :  {} ", mlpSolution.getSolutionId());
				Gson gson = new Gson();
				...
				logger.info("Store members (parent-child relationships) of composite solutions into CDS : Status : {}", flag);
			}
		} catch (Exception e) {
		...
		logger.info("4. Successfully updated the Cdump file for solution ID :  {} ", mlpSolution.getSolutionId());
		...
		try {
			...
			logger.info("5. Successfully uploaded the Cdump file for solution ID :  {} ", mlpSolution.getSolutionId());
			DSUtil.deleteFile(path.concat("acumos-cdump" + "-" + dscs.getcId()).concat(".json"));
			logger.info("5.1 Successfully deleted the local Cdump file for solution ID : " + mlpSolution.getSolutionId());
		...
		try {
			...
			logger.info("6. Successfully created the artifact for the cdumpfile for the solution : {0} artifact ID : {1}" ,mlpSolution.getSolutionId(), mlpArtifact.getArtifactId());
			...
			logger.info("7. Successfully associated the Solution Revision Artifact for solution ID  : " + mlpSolution.getSolutionId());
		} catch (Exception e) {
		...
		logger.info("insertCompositeSolution() End ");
	...
	public String updateCompositeSolution(DSCompositeSolution dscs) throws AcumosException, IOException, URISyntaxException {
		logger.info("updateCompositeSolution() Begin ");
		...
		if (null != mlpSolution) {
			...
			if (mlpSolution.getName().equalsIgnoreCase(dscs.getSolutionName())) {
				...
				if(null == mlpSolutionRevision){
					logger.info("Upadating Solution with the new version");
					...
					if (cdump.getVersion().equals(dscs.getVersion())) {
						...
						if (!dscs.getIgnoreLesserVersionConflictFlag()) {
							...
							logger.info("Upadating Existing Solution with the previous version");
							result = updateExistingSolution(mlpSolutionRevision, mlpSolution, dscs, cdump, cdumpFileName, path);
						}
					}
				}
		...
		logger.info("updateCompositeSolution() End ");
		return result;
	}
	...
	public String updateExistingSolution(MLPSolutionRevision mlpSR, MLPSolution mlpSolution, DSCompositeSolution dscs, Cdump cdump, String cdumpFileName, String path)
		...
		logger.info("updateExistingSolution() Start ");
		String result = "";
		...
		if("PB".equals(mlpSR.getAccessTypeCode()) || "OR".equals(mlpSR.getAccessTypeCode())){
			...
			if (null == cdump) {
				...
				for (MLPArtifact mlpArtifact : artfactsList) {
					...
					logger.info("mlpArtifact.getArtifactTypeCode()  :  {} ", mlpArtifact.getArtifactTypeCode());
					if (mlpArtifact.getArtifactTypeCode().equals(props.getArtifactTypeCode())) {
						logger.info("{0} = {1} ", mlpArtifact.getArtifactTypeCode(), props.getArtifactTypeCode());
						dscs.setCdump(cdumpArtifact);
						mlpArtifact.setUri(cdumpArtifact.getNexusURI());
						mlpArtifact.setModified(dateInstant);
						mlpArtifact.setSize(cdumpArtifact.getContentLength());
						cdmsClient.updateArtifact(mlpArtifact);
						logger.info("Successfully updated the artifact for the cdumpfile for the solution : {0} artifact ID : {1}" , mlpSolution.getSolutionId(),  mlpArtifact.getArtifactId());
		...
		logger.info("updateExistingSolution() End ");
		return result;
	}
	...
	public String updateSolnWithNewVersion(MLPSolution mlpSolution, DSCompositeSolution dscs) throws IOException, AcumosException {
		...
		logger.info("updateSolnWithNewVersion() Start ");
		String result = "";
		Date currentDate = new Date();
		Instant dateInstant = Instant.now();
		...
		try {
			...
			logger.info("Successfully Created the SolutionRevision :  {} ", mlpSolutionRevision.getRevisionId());
		} catch (Exception e) {
		...
		Artifact cdumpArtifact = new Artifact(cdumpFileName, "json", mlpSolution.getSolutionId(), dscs.getVersion(),
				path, payload.length());
		logger.info("Successfully updated the Cdump file for solution ID :  {} ", mlpSolution.getSolutionId());
		...
		try {
			...
			logger.info("Successfully created the artifact for the cdumpfile for the solution : {0} artifact ID : {1}" ,mlpSolution.getSolutionId() , mlpArtifact.getArtifactId());
			// 6.5 assocaite the artifact with solutionrevision using CDS.
			cdmsClient.addSolutionRevisionArtifact(mlpSolution.getSolutionId(), mlpSolutionRevision.getRevisionId(),
					mlpArtifact.getArtifactId());

			logger.info("Successfully associated the Solution Revision Artifact for solution ID  : " + mlpSolution.getSolutionId());
		} catch (Exception e) {
		...
		cdmsClient.updateSolution(mlpSolution);
		result = "{\"solutionId\": \"" + mlpSolution.getSolutionId() + "\", \"version\" : \"" + dscs.getVersion() + "\" }";
		logger.info("updateSolnWithNewVersion() End ");
		return result;
	}
	...
	private void uploadFilesToRepository(String solutionID, String revisionId, String version, Artifact artifact) throws AcumosException {
		logger.info(" uploadFilesToRepository() started ");
		...
		try {
			...
			logger.info("uploadFilesToRepository() ended ");
		} catch (Exception e) {
	...
	public boolean deleteCompositeSolution(String userId, String solutionId, String version)
		...
		try {
			...
			if (null != mlpSolution) {
					...
					if (mlpSolRevision.getVersion().equals(version) && mlpSolRevision.getUserId().equals(userId)) {
						...
						for (MLPArtifact mlpArtifact : mlpArtifacts) {
							...
							if ("CD".equals(mlpArtifact.getArtifactTypeCode())) {
								...
								logger.info("Successfully Deleted the SolutionRevisionArtifact which is if CDUMP i.e CD ");
								// Delete Artifact
								// 3. Delete the artifact "CD"
								cdmsClient.deleteArtifact(artifactId);
								logger.info("Successfully Deleted the CDUMP Artifact ");
							...
							if("MI".equals(mlpArtifact.getArtifactTypeCode())){
								...
								if(artifactURI){
									...
									logger.info("Successfully Deleted the SolutionRevisionArtifact which is if PROTO i.e MI");
									cdmsClient.deleteArtifact(artiId);
									logger.info("Successfully Deleted the .proto file Artifact ");
									nexusArtifactClient.deleteArtifact(mlpArtifact.getUri());
								}
							}
							if("BP".equals(mlpArtifact.getArtifactTypeCode())){
								...
								// Delete SolutionRevisionArtifact
								cdmsClient.dropSolutionRevisionArtifact(solutionId, revisionId, artifactId);
								logger.info("Successfully Deleted the SolutionRevisionArtifact which is if BLUEPRINT i.e BP");
								// Delete Artifact
								// 3. Delete the artifact "BP"
								cdmsClient.deleteArtifact(artifactId);
								logger.info("Successfully Deleted the BluePrint Artifact ");
								// 4. Delete the cdump file from the Nexus
								nexusArtifactClient.deleteArtifact(mlpArtifact.getUri());
							}
				...
				result = false;
				logger.info("Solution Not found :  {} ", solutionId);
			}
		} catch (Exception e) {
			result = false;
	...
	public String closeCompositeSolution(String userId, String solutionId, String solutionVersion, String cid) {
		...
		try {
			...
			if (userId == null) {
				...
				logger.info("Delete file :  {} ", path.concat(cdumpFileName));
				DSUtil.deleteFile(path.concat(cdumpFileName));
				logger.info("Delete User Dir :  {} ", path);
	...
	public String getCompositeSolutions(String userID, String visibilityLevel) throws AcumosException {
		...
		try {
			...
			logger.info("The Date Format :  {} ", confprops.getDateFormat());
			SimpleDateFormat sdf = new SimpleDateFormat(confprops.getDateFormat());
			...
			if (mlpSolutions == null) {
				...
				logger.info("CommonDataService returned Solution list of size :  {} ", mlpSolutions.size());
							...
							if (visibilityLevel.contains(mlpSolRevision.getAccessTypeCode())) {
								...
								if (null != mlpSolRevision.getAccessTypeCode()
									...
									if (mlpSolRevisions == null) {
										logger.info("CommonDataService returned null SolutionRevision list");
									} else if (mlpSolRevisions.isEmpty()) {
										logger.info("CommonDataService returned empty SolutionRevision list");
									} else {
										logger.info("CommonDataService returned SolutionRevision list of size : " + mlpSolRevisions.size());
	...
	public String validateCompositeSolution(String userId, String solutionName, String solutionId, String version)
		...
		try {
			...
			logger.info("1. Read the cdump file");
			...
			logger.info("2. get the Nodes from the cdump file and collect the nodeId's");
			...
			if (null != nodes && null != relationsList && !relationsList.isEmpty()) {
				...
				if(resultVo.getSuccess().equals("true")){
					...
					logger.info("On successful validation generate the BluePrint file.");
					...
					if (firstNode.getType().getName().equals(DATABROKER_TYPE)) {
						for (Relations relation : relationsList) {
							if (relation.getSourceNodeId().equals(firstNodeId)) {
								...
								logger.info("ByteArrayOutputStream of ProtoFile :" + bytes);
								if (null != bytes) {
									String protobufFile = bytes.toString();
									logger.info("protobufFile in String Format :" + protobufFile);
		...
		logger.info("validateCompositeSolution() in Service  : End ");
		return result;
	}
	...
	private void createAndUploadProtobuf(String userId, String solutionId, String version, String revisionId, String protoPayload, Cdump cdump)
		...
		try { 
			...
			logger.info("Error : Issue in createAndUploadProtobuf() : Failed to create the protobuf Artifact for Composite Solution ");
	...
	private String createAndUploadBluePrint(String userId, String solutionId, String solutionName, String version, Cdump cdump) throws ServiceException {
		...
		mapper.setSerializationInclusion(Include.NON_NULL);
		String path = DSUtil.readCdumpPath(userId, confprops.getToscaOutputFolder());
		logger.info("6. On successful validation generate the BluePrint file");
		BluePrint bluePrint = new BluePrint();
		// 7. Set the Solution name and version
		logger.info("7. On successful validation generate the BluePrint file");
		...
		logger.info("8. Get the nodes from Cdump file & set the required details in the blueprint nodes");
		...
		logger.info("9. Extract NodeId, NodeName,NodeSolutionId,NodeVersion");
		for (Nodes n : cdumpNodes) {
			...
			logger.info("11. Get the MlpSolutionRevisions from CDMSClient for the NodeSolutionId");
			...
			if (n.getType().getName().equalsIgnoreCase(props.getGdmType())) {
				logger.info("GDM Found :  {} ", n.getNodeId());
				// For Generic Data Mapper, get the dockerImageUrl by deploying the GDM Construct the image for the Generic Data mapper
				logger.info("For Generic Data Mapper, get the dockerImageUrl by deploying the GDM Construct the image for the Generic Data mapper");
				dockerImageURL = gdmService.createDeployGDM(cdump, n.getNodeId(), userId);
				if (null == dockerImageURL) {
					logger.info("Error : Issue in createDeployGDM() : Failed to create the Solution Artifact ");
				...
				logger.info("DataBroker Found :  {} ", n.getType().getName());
				dockerImageURL = dbService.getDataBrokerImageURI(n);
				if (null == dockerImageURL) {
					logger.info("Error : Issue in createDeployDataBroker() : Failed to create the Solution Artifact ");
					throw new ServiceException("  Issue in createDeployGDM() ", "333",
				...
				logger.info("12. Get the list of artifact from CDMSClient which will return the DockerImageUrl");
				dockerImageURL = getDockerImageURL(nodeSolutionId, mlpSolRevision);
			}
			// 13. Set the values in the bluePrint Node
			logger.info("13. Set the values in the bluePrint Node");
			bpnode = new Node();
		...
		try {
			...
			logger.info("19. Upload the file to Nexus");
			...
			logger.info("21. Get the SolutionRevisions from CDMSClient.");
			...
			logger.info("22. Iterate over MLPSolutionRevisions and get the CompositeSolutionRevision.");
			...
			logger.info("23. Creating the Artifact from CDMSClient.");
			...
			logger.info("ArtifactFlag for BP : " + bluePrintExists);
			if (bluePrintExists) {

				// 24. Update the Artifact which is already exists as BP
				cdmsClient.updateArtifact(mlpArtifact);
				logger.info("24. Updated the ArtifactTypeCode BP which is already exists");
			} else {
				mlpArtifact = cdmsClient.createArtifact(mlpArtifact);

				logger.info("Successfully created the artifact for the BluePrint for the solution : "
								+ solutionId + " artifact ID : " + mlpArtifact.getArtifactId());

				// 25. Associate theSolutionRevisionArtifact for solution ID.

				logger.info("25. Associate the SolutionRevisionArtifact for solution ID.");

				cdmsClient.addSolutionRevisionArtifact(solutionId,compositeSolutionRevision.getRevisionId(), mlpArtifact.getArtifactId());

				logger.info("Successfully associated the Solution Revision Artifact for solution ID  : "
								+ solutionId);
			}
	...
	public SuccessErrorMessage setProbeIndicator(String userId, String solutionId, String version, String cid,
		...
		logger.info("setProbeIndicator() : Begin ");
        String id = "";
		Gson gson = new Gson();
		ObjectMapper mapper = new ObjectMapper();
		try {
        ...
        logger.info("setProbeIndicator() : End ");
