		...
		try {
			...
			if (null != nodes && null != relationsList && !relationsList.isEmpty()) {
				...
				if(resultVo.getSuccess().equals("true")){
					...
					for (Nodes no : nodes) {
						if (no.getType().getName().equals(DATABROKER_TYPE)) {
							if (no.getNodeId().equals(firstNodeId)) {
								for(Relations rel : relationsList){
									String sourceNodeId = rel.getSourceNodeId();
									if(sourceNodeId.equals(firstNodeId)){
										String targetNodeId = rel.getTargetNodeId();
										for(Nodes n : nodes){
											if(n.getNodeId().equals(targetNodeId)){
												...
												logger.debug(EELFLoggerDelegator.debugLogger,"String Format of protobufFile :" + protobufFile);
												Property[] prop = no.getProperties();
												for(Property p : prop){
													p.getData_broker_map().setProtobufFile(protobufFile);
												}
												no.setProperties(prop);
											}
										}
									}
								}
