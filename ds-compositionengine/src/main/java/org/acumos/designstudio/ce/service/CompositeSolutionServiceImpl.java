...
public class CompositeSolutionServiceImpl implements ICompositeSolutionService {
	...
	public String validateCompositeSolution(String userId, String solutionName, String solutionId, String version)
		...
		try {
			...
			if (null != nodes && !nodes.isEmpty() && null != relationsList && !relationsList.isEmpty()) {
				...
				if (CollectionUtils.isEqualCollection(idList, set)) {
					...
					if (relationsList.size() >= idList.size() - 1) {
						//  Case 1. Check Data Broker is present or not
							...
							if ("DataBroker".equals(type)) {
								...
								for (Relations rel : relationsList) {
									if (!nodeid.equals(rel.getTargetNodeId())) {
										// For DataBroker if the targetNode is not connected to any other nodeId
										// then solution first Node is DataBroker
										isDataBroker = true;
										break;
									} else {
						...
						if (!isDataBroker) {
							...
							// If DataBroker is true then check for one to one
							// mapping for the SourceNodeId and TargetNodeId
							...
							for (Relations rel : relationsList) {
								// Case 2. If Duplicate SourceNodeRequirements's are
								// found then solution is Invalid
								...
								// Case 3. If Duplicate getTargetNodeCapability's are
								// found then solution is Invalid
							...
							if (isConnectedToMultiple) {
								// If Flag is true means duplicate Source or TargetID found then its 
								// Invalid Solution and give error message like below
