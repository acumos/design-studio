...
public class TagImageCommand extends DockerCommand {
	...
	public void execute() throws DockerException {
		...
		try {
			logger.info(EELFLoggerDelegator.applicationLogger, "start tagging image " + image + " in " + repository + " as " + tag);
			client.tagImageCmd(image, repository, tag).withForce(withForce).exec();
			logger.info(EELFLoggerDelegator.applicationLogger, "Tagged image " + image + " in " + repository + " as " + tag);
		} catch (NotFoundException e) {
			if (!ignoreIfNotFound) {
				logger.error(EELFLoggerDelegator.errorLogger, String.format("image '%s' not found ", image));
				throw e;
			} else {
				logger.info(EELFLoggerDelegator.applicationLogger, String.format(
