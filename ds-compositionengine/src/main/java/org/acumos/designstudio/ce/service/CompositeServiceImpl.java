...
public class CompositeServiceImpl implements ICompositeSolutionService {

	private static EELFLoggerDelegator logger = EELFLoggerDelegator.getLogger(CompositeServiceImpl.class);
	...
	@Override
	public String saveCompositeSolution(DSCompositeSolution dscs) throws CustomException{
	...
	private String insertCompositeSolution(DSCompositeSolution dscs) throws CustomException, IOException {
	 ...
	 */
	public String updateCompositeSolution(DSCompositeSolution dscs) throws CustomException, IOException {
		...
		if (null != mlpSolution) {
			...
			if (mlpSolution.getName().equalsIgnoreCase(dscs.getSolutionName())) {
				...
				// New Case: When user tries to update the existting solution with a different name
	...
	private void uploadFilesToRepository(String solutionID, String version, Artifact a) throws CustomException {
	...
	public String validateCompositeSolution(String userId, String solutionName, String solutionId, String version) {
		...
		try {
			...
			logger.error(EELFLoggerDelegator.errorLogger, "------------Exception Message : " + e.getMessage());
		}
	...
	private List<Depends_On> getRelations(Cdump cdump, String nodeId) {
		...
		for (Relations cr : cRelations) {
			if (cr.getSourceNodeId().equals(nodeId)) {
				...
				System.out.println("********************** operation_signature : " + operation);
				depends_on.setOperation_signature(os);
				//depends_on.setOperation_signature(operation_signature);
				// add depends_on object to dependsOnList List
