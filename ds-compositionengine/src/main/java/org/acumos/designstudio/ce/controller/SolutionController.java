...
public class SolutionController {
	...
	public String readCompositeSolutionGraph(@RequestParam(value = "userId", required = true) String userId,
		...
		String result = solutionService.readCompositeSolutionGraph(userId, solutionId, version);
	 ...
	 * @throws AcumosException 
	 */
	@ApiOperation(value = "JarDockerization operation")
	@RequestMapping(value = "/jarDockerization", method = RequestMethod.POST)
	public String jarDockerization(@RequestParam(value = "jarFile") MultipartFile jar,
			@RequestParam(value = "dockerFile") MultipartFile dockerFile) throws AcumosException {

		logger.debug(EELFLoggerDelegator.debugLogger,
				"------- jarDockerization() in SolutionController begins --------");
		File outputFolder = new File("tmp", "dockerization");
		outputFolder.mkdirs();
		File localJarFile = new File(outputFolder, jar.getOriginalFilename());
		try {
			DSUtil.copyFile(jar.getInputStream(), localJarFile);

		} catch (IOException e) {
			logger.error(EELFLoggerDelegator.errorLogger, "---- Exception in jarDockerization() -----", e);
		}

		File localDockerFile = new File(outputFolder, dockerFile.getOriginalFilename());
		try {
			DSUtil.copyFile(jar.getInputStream(), localDockerFile);

		} catch (IOException e) {
			logger.error(EELFLoggerDelegator.errorLogger, "---- Exception in jarDockerization() -----", e);
		}
		logger.debug(EELFLoggerDelegator.debugLogger, "Started docker client");
		DockerClient dockerClient = DockerClientFactory.getDockerClient(dockerConfiguration);
		logger.debug(EELFLoggerDelegator.debugLogger, "Docker client created successfully");
		String imageId = null;
		try {
			logger.debug(EELFLoggerDelegator.debugLogger, "Docker image creation started");
			CreateImageCommand createCMD = new CreateImageCommand(outputFolder, "Model", "1.0.0", null, false, true);
			createCMD.setClient(dockerClient);
			createCMD.execute();
			logger.debug(EELFLoggerDelegator.debugLogger, "Docker image creation done");
			imageId = createCMD.getImageId();
			logger.debug(EELFLoggerDelegator.debugLogger, "Docker imageId : " + imageId);
		} catch (DockerException e) {
			logger.error(EELFLoggerDelegator.errorLogger, "---- Exception in jarDockerization() -----", e);
		}
		logger.debug(EELFLoggerDelegator.debugLogger, "------- jarDockerization() in SolutionController ends --------");
		return imageId;
	}
