...
import org.acumos.sqldatabroker.util.Constants;
...
public class DataSourceFactory {
	...
	public BasicDataSource getDataSource(Configuration conf) throws ServiceException {
		...
		try {
			...
			if (Constants.MYSQLCLASSNAME.equals(conf.getDataSourceClassName())) {
				ds = new BasicDataSource();
				ds.setDriverClassName(Constants.MYSQLCLASSNAME);
			} else if (Constants.ORACLECLASSNAME.equals(conf.getDataSourceClassName())) {
				ds = new BasicDataSource();
				ds.setDriverClassName(Constants.ORACLECLASSNAME);
			}
			if (null != ds) {
