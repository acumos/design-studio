...
public class CSVDatabrokerController {
	...
	@ApiOperation(value = "Set the environment configuration.")
	...
	public Object configureEnvironment(@RequestBody Configuration conf, HttpServletResponse response) {
		...
		try {
			...
			//2. process protobuf 
			ProtobufServiceImpl.getInstance().processProtobuf(conf);
			...
			result = new Result(HttpServletResponse.SC_EXPECTATION_FAILED, "Error while setting Environment configuration");
		}
		return result;
	}
	...
	public Object pullData() {
		...
		int resultsetSize = ConfigurationService.getInstance().getResultsetSize();
		if(resultsetSize == 1){
			byte[] result = null;
			...
			try {
				...
				return new Result(HttpServletResponse.SC_EXPECTATION_FAILED, "No Data Found !!!");
			}
			return result;
        ...
        return new StreamingResponseBody() {
            ...
            public void writeTo (OutputStream out) throws IOException {
            	try {
            		...
            		out.write((new Result(HttpServletResponse.SC_EXPECTATION_FAILED, e.getMessage()).toString()).getBytes());
            	}
            }
        };
		} else {
