...
public class ProtobufUtil {
	private final static EELFLoggerDelegator logger = EELFLoggerDelegator.getLogger(ProtobufUtil.class);

	/**
	 * This method return the DynamicMessage.Builder for the provided input
	 * msgstructure
	 * 
	 * @param msg
	 *            This method accepts msg
	 * @return msgBuilder This method returns msgBuilder
	 */
	...
	public static Protobuf parseProtobuf(String protobufStr) {
		...
		try {
			...
			logger.error(EELFLoggerDelegator.errorLogger, "Error in parseProtobuf(String protobufStr)", e);
		} finally {
			if (null != scanner) {
				scanner.close();
			}
	...
	public static ProtobufMessage parseMessage(String messageStr) {
		...
		try {
			...
			logger.error(EELFLoggerDelegator.errorLogger, "Error in  parseMessage(String messageStr)", e);
		} finally {
			if (null != scanner) {
				scanner.close();
			}
	...
	private static ProtobufService parserService(String serviceStr) {
		...
		while (scanner.hasNextLine()) {
			...
			logger.error(EELFLoggerDelegator.errorLogger, "Error in  parserService(String serviceStr)", e);
		}
		finally {
			scanner.close();
		}
