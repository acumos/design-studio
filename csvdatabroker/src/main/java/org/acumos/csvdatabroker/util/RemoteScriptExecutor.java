...
public class RemoteScriptExecutor {
	...
	private void connect(String channelType) throws Exception {
		...
		try {
			...
			throw new ServiceException("Not able to Connect Remote server ","401", "Not able to Connect Remote server ");
		}
	}
	...
	public void createshellFile(String script) throws Exception {
		...
		try {
			...
			throw new ServiceException("Not able to create shell File ","401", "Not able to create shell File");
		} finally {
			if(null != writer){
				try {
					writer.close();
	...
	public byte[] executeShell(int start) throws Exception {
		...
		try{
            ...
            while ((line = reader.readLine()) != null) {
            	...
            	if(cnt == start ){
            		output = ProtobufService.getInstance().convertToProtobufFormat(line);
            		break;
            	}
         ...
         // Command execution completed here.
			...
			throw new ServiceException("Not able to execute shell File ","401", "Not able to execute shell File");
		} finally {
			if(null != reader){
				try {
					reader.close();
